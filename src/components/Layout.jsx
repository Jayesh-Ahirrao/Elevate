...
export default function Layout({ children }) {
  ...
  const drawer = (
    <div>
      <Toolbar>
        <img src={logo} alt="Logo" style={{ maxWidth: "150px", margin: "0 auto", display: "block" }} />
      </Toolbar>
      <Divider />
      <List>
        {menuItems.map((item) => (
      ...
      <AppBar position="fixed" sx={{ width: { sm: `calc(100% - ${drawerWidth}px)` }, ml: { sm: `${drawerWidth}px` }, bgcolor: "white", color: "black" }}>
        <Toolbar>
          ...
          <Box sx={{ display: "flex", alignItems: "center", ml: "auto", gap: 2 }}>
            {user ? (
              <Box sx={{ display: "flex", alignItems: "center", cursor: "pointer" }} onMouseEnter={(e) => setAnchorEl(e.currentTarget)} onMouseLeave={() => setAnchorEl(null)}>
                ...
                <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)} PaperProps={{ sx: { maxWidth: 180, mt: 1.5 } }}>
